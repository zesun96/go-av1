...
func main() {
    ...
    enc, err := av1.NewEncoder(av1.EncoderOptions{
        ...
        BitDepth: 8, Chroma: av1.Chroma420,
        CRF: 30, Preset: 13,
    })
    if err != nil {
        log.Fatal(err)
    ...
    pic := &av1.Picture{
        ...
        StrideY: width, StrideUV: width / 2,
        Width: width, Height: height,
        BitDepth: 8, Chroma: av1.Chroma420,
    }
    ...
    // EncodedPacket.Data is one AV1 temporal unit. A container writer can
    // mux it into IVF, WebM, or another AV1-capable container.
    if _, err := os.Stdout.Write(packet.Data); err != nil {
        log.Fatal(err)
    }
}
```

For Y4M-to-IVF command-line encoding, see
[`cmd/go-av1enc`](cmd/go-av1enc).
