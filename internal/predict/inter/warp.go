...
// PrepWarpAffine produces AV1's Q4 compound intermediate for affine motion.
func PrepWarpAffine(tmp []int16, tmpStride int, src []uint8, srcStride, srcW, srcH int,
	bx, by, bw, bh, ssHor, ssVer int, matrix [6]int32, abcd [4]int16) {
	for y := 0; y < bh; y += 8 {
		srcY := (by << ssVer) + ((y + 4) << ssVer)
		mat3Y := int64(matrix[3])*int64(srcY) + int64(matrix[0])
		...
		for x := 0; x < bw; x += 8 {
			...
			mx := ((int(mvx) & 0xffff) - int(abcd[0])*4 - int(abcd[1])*7) &^ 0x3f
			my := ((int(mvy) & 0xffff) - int(abcd[2])*4 - int(abcd[3])*4) &^ 0x3f
			prepWarp8x8(tmp[y*tmpStride+x:], tmpStride, src, srcStride, srcW, srcH,
				dx, dy, minWarp(8, bw-x), minWarp(8, bh-y), abcd, mx, my)
		}
	}
}

func prepWarp8x8(tmp []int16, tmpStride int, src []uint8, srcStride, srcW, srcH,
	...
	for y := 0; y < outH; y++ {
		...
		for x := 0; x < outW; x++ {
			...
			tmp[y*tmpStride+x] = int16((sum + 64) >> 7)
			tmy += int(abcd[2])
		}
		my += int(abcd[3])
	}
