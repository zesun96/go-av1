...
// AV1 dequantization tables and functions, ported 1:1 from
// dav1d/src/dequant_tables.c and dav1d/src/decode.c init_quant_tables.

// QindexRange is the number of entries in the quantizer index table,
// mirroring dav1d QINDEX_RANGE.
const QindexRange = 256

// DqTbl is the constant dequantization lookup table, mirroring
// dav1d_dq_tbl[3][QINDEX_RANGE][2]. Indexing:
//
//	DqTbl[bitDepth][qindex][0] = DC dequant value
//	DqTbl[bitDepth][qindex][1] = AC dequant value
//
// bitDepth: 0=8-bit, 1=10-bit, 2=12-bit (mirrors dav1d seq_hdr->hbd).
var DqTbl = [3][QindexRange][2]uint16{
	// 8-bit
	{
		{4, 4}, {8, 8}, {8, 9}, {9, 10},
...
// IclipU8 clips v to [0, 255], mirroring dav1d iclip_u8.
func IclipU8(v int) int {
	if v < 0 {
		return 0
	}
	if v > 255 {
...
// QuantDeltas holds the per-plane delta values from the frame header,
// mirroring the Av1FrameHeader.quant structure.
type QuantDeltas struct {
	YdcDelta int // signed delta for Y DC
	UdcDelta int // signed delta for U DC
	UacDelta int // signed delta for U AC
	VdcDelta int // signed delta for V DC
	VacDelta int // signed delta for V AC
}

// SegDeltaQ holds per-segment delta_q values, mirroring
// Av1FrameHeader.segmentation.seg_data.d[i].delta_q.
// If segmentation is not enabled, the caller should pass a nil slice.
type SegDeltaQ = []int

// InitQuantTables computes the per-segment, per-plane dequantization values
// from the base quantizer index, frame-level deltas, and optional
// segmentation deltas. The return value is indexed as:
//
//	result[seg][plane][0] = DC dequant
//	result[seg][plane][1] = AC dequant
//
// where plane: 0=Y, 1=U, 2=V.
//
// This mirrors dav1d init_quant_tables() in decode.c:54-74.
func InitQuantTables(qidx int, d QuantDeltas, hbd int, segDeltaQ SegDeltaQ) [8][3][2]uint16 {
	var dq [8][3][2]uint16
	nSeg := 1
	if segDeltaQ != nil {
		nSeg = 8
	...
	for i := 0; i < nSeg; i++ {
		yac := qidx
		if segDeltaQ != nil {
			yac = IclipU8(qidx + segDeltaQ[i])
		}
		ydc := IclipU8(yac + d.YdcDelta)
		udc := IclipU8(yac + d.UdcDelta)
		uac := IclipU8(yac + d.UacDelta)
		vdc := IclipU8(yac + d.VdcDelta)
		vac := IclipU8(yac + d.VacDelta)
...
// Dequant applies AV1 dequantization to a coefficient block in-place.
//
// The coefficients are assumed to be stored in a flat array of width w
// (4*TxfmDim.W) with the given stride. The DC coefficient is at [0];
// all others are AC. The eob parameter indicates the last non-zero
// coefficient index (in raster scan order); if eob<0, all coefficients
// are processed.
//
// The dq parameter supplies [DC_dequant, AC_dequant]. The txSz parameter
// is the RectTxfmSize index used to derive the dequant shift
// (dq_shift = max(0, ctx - 2), mirroring dav1d).
//
// The optional qm (quality matrix) parameter is used only for non-identity
